...
def write_configs_package(config_file, cache_directory, noisily, rerun):
    """
...
def load_config_package(config_file):
    ...
    with open(config_file, 'r') as f:
        ...
        config_dets = list(itertools.chain.from_iterable(config_dets))
        idx1 = config_dets.index('Cache Directory:')
        idx2 = config_dets.index('Noisily:')
        idx3 = config_dets.index('Rerun:')
        directory = single_item(config_dets[idx1+1:idx2])
        noisily = single_item(config_dets[idx2+1:idx3])
        rerun = single_item(config_dets[idx3+1:])
